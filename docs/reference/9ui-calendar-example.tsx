// Reference UI for the visit-booking screen.
// Source: https://github.com/borabaloglu/9ui/blob/main/apps/www/src/components/examples/calendar.tsx
// 9ui (https://9ui.dev) is a shadcn-style registry built on Base UI (not Radix);
// components install via the shadcn CLI from the 9ui registry.
//
// Adaptations for the visits app (see SPEC.md → "UI"):
// - Left panel: roster title + pinned admin note + today's visitors, instead of
//   host/meeting info. Drop the timezone Select entirely (everyone is in nl-NL).
// - Calendar: don't disable weekends; mark days that already have visits with a
//   dot, and highlight upcoming days with NO visits (gaps are the feature).
// - Slot list: show booked visits inline ("15:00 — Tom & Els") between free
//   slots; free slots come from visiting hours minus blocked times minus slots
//   at maxConcurrent capacity.
// - Confirm dialog: name + optional note only. No email, no "add guests".
// - Own bookings (matched via localStorage editToken) render edit/cancel instead
//   of a book button.
